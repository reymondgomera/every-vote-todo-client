...
const LoginForm = () => {
  ...
  useEffect(() => {
    if (data) window.location.assign("/dashboard");
  }, [data]);
    ...
    <div className="max-w-[400px] rounded-lg bg-white p-10 shadow-sm md:border md:border-input">
      <header className="flex flex-col items-center justify-center gap-5">
        ...
        <div className="flex flex-col items-center justify-center gap-2">
          <h1 className="text-center text-2xl font-extrabold dark:text-slate-950">Login</h1>
          <p className="text-center text-sm leading-relaxed text-muted-foreground">
            Securely access your EveryVote Todo account and get back to achieving your goals!
          </p>
        </div>
      </header>
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="mt-8 space-y-4">
          ...
          <p className="text-center text-xs text-muted-foreground">
            ...
            <Link to="/register" className="hover:text-primary hover:underline hover:decoration-primary">
              Register
            </Link>
          </p>
